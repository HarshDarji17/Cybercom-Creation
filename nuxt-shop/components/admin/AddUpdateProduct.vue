...
export default {
  ...
  data: () => ({
    categories: ['furniture', 'Sportsshoes', 'fashion', 'mobile', 'laptop'],
    subCategories: [
      'LivingRoom',
      'Bedroom',
      'DiningRoom',
  ...
  computed: {
    ...
    imgErrors() {
      ...
      !this.$v.product.img.required && errors.push('Image URL is required.')
      return errors
    },
    brandNameErrors() {
      const errors = []
    ...
    ratingErrors() {
      ...
      !this.$v.product.rating.required && errors.push('Rating is required.')
      return errors
    },
  },
  methods: {
    addProd(product) {
      this.$v.$touch()
      if (!this.$v.$invalid) {
        var newProductRef = db
          .collection(`products/${product.category}/${product.subCategory}`)
          .doc()
        newProductRef
          .set({
            id: newProductRef.id,
        ...
        this.snackbar = true
        this.snackbarText = ' You are add product sucessfully'
      }
    },
    updateProd(product) {
    ...
    clear() {
      ...
      this.snackbar = true
      this.snackbarText = ' You are clear the products table sucessfully'
    },
  },
}
</script>
